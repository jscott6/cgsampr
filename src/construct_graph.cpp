...
using namespace ConstructGraph;
...
Graph::Graph(IV out_degree, IV in_degree, IM fixed)
{
  ...
  adjacency_list_ = vector<vector<int> >(nvertices);
  flow_ = vector<vector<int> >(nvertices, vector<int>(nvertices,0));
  capacity_ = vector<vector<int> >(nvertices, vector<int>(nvertices,0));


  // initialise edges between source_ and rows
  for (int i = 0; i != m_; ++i)
  {
    addEdge(source_, i + 1, out_degree(i));
  ...
  for (int j = 0; j != n_; ++j)
  {
    ...
    addEdge(m_ + j + 1, sink_, in_degree(j));
  }
...
IM Graph::constructWeightMatrix()
{
    IM x(m_,n_);
    for(int i = 0; i != m_; ++i)
        for(int j = 0; j != n_; ++j)
